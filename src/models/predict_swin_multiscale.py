"""
Generate test predictions using the SwinMultiScale model with optional TTA.

Usage:
    python -m src.models.predict_swin_multiscale
    
    # With specific options:
    python -m src.models.predict_swin_multiscale --no-tta
    python -m src.models.predict_swin_multiscale --checkpoint /path/to/weights.pth
"""
...
def find_best_checkpoint(model_name: str = "swin_multiscale") -> Optional[Path]:
    """Find the best checkpoint for the model."""
    # Priority 1: Check analysis_outputs/models for best checkpoint
    best_path = OUTPUT_CONFIG.models_root / f"{model_name}_best.pth"
    if best_path.exists():
        return best_path
    
    # Priority 2: Check for regular weights
    weights_path = OUTPUT_CONFIG.models_root / f"{model_name}_weights.pth"
    if weights_path.exists():
        return weights_path
    
    # Priority 3: Check training_logs
...
def predict_batch(model: nn.Module, loader: DataLoader, device: torch.device):
    ...
    for batch in tqdm(loader, desc="Predicting", unit="batch", dynamic_ncols=True):
        ...
        # Model returns only main logits in eval mode
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate predictions with SwinMultiScale")
    parser.add_argument("--model-name", type=str, default="swin_multiscale", help="Model name")
    parser.add_argument("--no-tta", action="store_true", help="Disable Test-Time Augmentation")
    parser.add_argument("--batch-size", type=int, default=48, help="Batch size (only without TTA)")
    parser.add_argument("--num-workers", type=int, default=4, help="DataLoader workers")
    parser.add_argument("--checkpoint", type=str, default=None, help="Path to specific checkpoint")
    parser.add_argument("--fusion-dim", type=int, default=512, help="Fusion dimension (must match training)")
    ...
    if ckpt_path is None or not ckpt_path.exists():
        raise FileNotFoundError(
            ...
            f"Run training first: python -m src.models.swin_multiscale"
        )
    
    # Load model (set use_aux_heads=False for inference - we only need main head)
    model = build_swin_multiscale(
        num_classes=11,
        pretrained=False,  # We're loading weights manually
        use_aux_heads=False,  # Not needed for inference
