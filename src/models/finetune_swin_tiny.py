"""
Fine-tuning script for Swin-Tiny to improve competition score.
...
- Gradient clipping
- Progressive layer unfreezing (optional)
...
Usage:
    ...
    python -m src.models.finetune_swin_tiny --epochs 30 --lr 1e-4 --mixup 0.4 --cutmix 0.4
"""
...
class FinetuneConfig:
    """Configuration for fine-tuning Swin-Tiny."""
    model_name: str = "swin_tiny_finetuned"
    base_model_name: str = "swin_tiny"  # For loading pretrained weights
    ...
    unfreeze_schedule: List[int] = field(default_factory=lambda: [0, 5, 10])  # Epochs to unfreeze more layers
...
def build_swin_tiny_finetune(num_classes: int, drop_path_rate: float = 0.1) -> nn.Module:
    """Build Swin-Tiny with configurable stochastic depth."""
...
def get_transforms(config: FinetuneConfig, is_training: bool) -> transforms.Compose:
    ...
    if is_training:
        ...
        if config.use_randaug:
            # RandAugment for grayscale
            transform_list.append(transforms.RandomAutocontrast(p=0.3))
            transform_list.append(transforms.RandomEqualize(p=0.3))
            transform_list.append(transforms.RandomAdjustSharpness(sharpness_factor=2, p=0.3))
...
def freeze_layers(model: nn.Module, freeze_until: str = "layers.2") -> None:
    """Freeze early layers of the model for progressive unfreezing."""
...
def get_layer_groups(model: nn.Module) -> List[List[nn.Parameter]]:
    ...
    # Swin structure: patch_embed, layers.0, layers.1, layers.2, layers.3, head
    groups = {
        "patch_embed": [],
        "layers_0_1": [],
        "layers_2_3": [],
    ...
    for name, param in model.named_parameters():
        if "patch_embed" in name:
            groups["patch_embed"].append(param)
        elif "layers.0" in name or "layers.1" in name:
            groups["layers_0_1"].append(param)
        elif "layers.2" in name or "layers.3" in name:
            groups["layers_2_3"].append(param)
        else:
            groups["head"].append(param)
    
    return [groups["patch_embed"], groups["layers_0_1"], groups["layers_2_3"], groups["head"]]
...
def evaluate(
    ...
    dataset: Optional[OrganDatasetFinetune] = None,
...
def run_finetuning(config: FinetuneConfig) -> None:
    ...
    # Build model
    model = build_swin_tiny_finetune(num_classes, config.drop_path_rate)
    ...
    # Optimizer with layer-wise LR (higher LR for head)
    layer_groups = get_layer_groups(model)
    param_groups = [
        {"params": layer_groups[0], "lr": config.lr * 0.1},  # patch_embed: lowest LR
        {"params": layer_groups[1], "lr": config.lr * 0.3},  # early layers
        {"params": layer_groups[2], "lr": config.lr * 0.6},  # later layers
        {"params": layer_groups[3], "lr": config.lr},        # head: full LR
    ]
    ...
    for epoch in range(config.epochs):
        ...
        if epoch in config.unfreeze_schedule:
            if epoch == 0:
                freeze_layers(model, "layers.2")
                print(f"[finetune] Epoch {epoch}: Freezing early layers (up to layers.2)")
            elif epoch == config.unfreeze_schedule[1] if len(config.unfreeze_schedule) > 1 else None:
                freeze_layers(model, "layers.0")
                print(f"[finetune] Epoch {epoch}: Unfreezing more layers (up to layers.0)")
            else:
                unfreeze_all(model)
                print(f"[finetune] Epoch {epoch}: Unfreezing all layers")
        ...
        for _ in range(n_iter_per_epoch):
            scheduler.step_update(epoch * n_iter_per_epoch + _)
    ...
    preds_tta, labels_tta = predict_with_tta(model, val_loader, device, tta_transforms, val_dataset)
    ...
    plt.subplot(1, 3, 3)
    plt.bar(["Best Val", "Best EMA", "TTA"], [best_val_acc, best_ema_acc, tta_accuracy])
    plt.ylabel("Accuracy")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fine-tune Swin-Tiny for improved competition score")
    ...
    parser.add_argument("--no-randaug", action="store_true", help="Disable RandAugment")
