"""
Generate test predictions using the fine-tuned ConvNeXt-Tiny model with TTA.

Usage:
    python -m src.models.predict_convnext_finetune
    
    # With specific options:
    python -m src.models.predict_convnext_finetune --no-tta --use-ema
"""
...
def build_model(num_classes: int = 11, drop_path_rate: float = 0.2) -> nn.Module:
    """Build ConvNeXt-Tiny model for inference."""
...
def find_best_checkpoint(model_name: str = "convnext_tiny_finetuned") -> Optional[Path]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate predictions with fine-tuned ConvNeXt-Tiny")
    parser.add_argument("--model-name", type=str, default="convnext_tiny_finetuned", help="Model name")
    ...
    # Device - ConvNeXt works better on CUDA, handle MPS carefully
    ...
    if ckpt_path is None or not ckpt_path.exists():
        raise FileNotFoundError(
            ...
            f"Run fine-tuning first: python -m src.models.finetune_convnext_tiny"
        )
